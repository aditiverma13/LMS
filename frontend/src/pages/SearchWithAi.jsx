...
function SearchWithAi() {
  ...
  // -------------------- STATE --------------------
  const [input, setInput] = useState(""); // User search input
  const [recommendations, setRecommendations] = useState([]); // AI search results
  const [listening, setListening] = useState(false); // Speech recognition state

  const startSound = new Audio(start); // Sound to play on mic start
  ...
  if (!recognition) console.log("Speech recognition not supported");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black to-gray-900 text-white flex flex-col items-center px-4 py-16">
      ...
      <div className="bg-white shadow-xl rounded-3xl p-6 sm:p-8 w-full max-w-2xl text-center relative">
        <FaArrowLeftLong
          className="text-black w-6 h-6 cursor-pointer absolute left-4 top-4"
          onClick={() => navigate("/")}
        />

        <h1 className="text-2xl sm:text-3xl font-bold text-gray-600 mb-6 flex items-center justify-center gap-2">
          <img src={ai} className="w-8 h-8 sm:w-10 sm:h-10" alt="AI" />
      ...
      {recommendations.length > 0 ? (
        <div className="w-full max-w-6xl mt-12 px-2 sm:px-4">
          <h2 className="text-xl sm:text-2xl font-semibold mb-6 text-white text-center flex items-center justify-center gap-3">
            <img src={ai1} className="w-10 h-10 sm:w-16 sm:h-16 p-2 rounded-full" alt="AI Results" />
            AI Search Results
          </h2>
