...
import { FaArrowLeft, FaLock, FaPlayCircle, FaStar } from "react-icons/fa";
...
const handleEnroll = async (courseId, userId) => {
  try {
    ...
    if (!orderResponse.data || !orderResponse.data.id) {
      ...
      console.error("Invalid order data:", orderResponse.data);
      return;
    }

    // 2️⃣ Configure Razorpay options
    const options = {
      key: import.meta.env.VITE_RAZORPAY_KEY_ID, // Must match frontend .env
      amount: orderResponse.data.amount,          // in paisa
      currency: "INR",
      name: "EduCrafted",
    ...
    rzp.on("payment.failed", function (response) {
      ...
      toast.error("Payment failed. Please check your key or try again.");
    });
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="w-full md:w-1/2">
            ...
            <img
              ...
              className="rounded-xl w-full object-cover mt-2"
            />
          </div>

          {/* Course Info */}
          <div className="flex-1 space-y-2 mt-5 md:mt-0">
            ...
            {!isEnrolled ? (
              ...
              <button
                className="bg-green-200 text-green-600 px-6 py-2 rounded mt-3"
        ...
        <div className="mt-8 border-t pt-6">
            ...
            <div className="flex gap-1 mb-2">
              {[1, 2, 3, 4, 5].map((star) => (
                <FaStar
                  ...
                  className={star <= rating ? "fill-yellow-500 cursor-pointer" : "fill-gray-300 cursor-pointer"}
                />
              ))}
            </div>
            <textarea
          ...
          <div className="flex items-center gap-4 pt-4 border-t">
            ...
            <div>
              ...
              <p className="text-gray-600 text-[10px] md:text-sm">{creatorData?.description}</p>
              <p className="text-gray-600 text-[10px] md:text-sm">{creatorData?.email}</p>
            </div>
          </div>
